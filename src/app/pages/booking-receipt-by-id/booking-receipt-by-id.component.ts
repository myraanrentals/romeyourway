...
@Component({
  selector: 'app-booking-receipt-by-id',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './booking-receipt-by-id.component.html',
  styleUrl: './booking-receipt-by-id.component.scss',
})
export class BookingReceiptByIdComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.paymentDetails.bookingId = bookingID;
    this._bookingService.getBookingDetails(bookingID).subscribe({
      next: (res) => {
        ...
        this.bookingDetails = row;
        this.name = row?.customeName || row?.customerName;
        this.email = row?.customerEmailId || row?.customerEmail;
        const dial = row?.countryDialCode ?? '';
        const mobile = row?.customerMobile ?? row?.customerPhone ?? '';
        this.phone = `${dial}${mobile}`;
        this.paymentDetails = {
          ...this.paymentDetails,
          orderId: String(row?.bookingId ?? bookingID),
          paymentCurrency: 'INR',
          paymentAmount: String(
            row?.actualAmount ?? row?.bookingAmount ?? row?.totalAmount ?? '',
          ),
          cashfreePaymentId: String(
            row?.cashfreePaymentId ?? row?.paymentReference ?? row?.transactionId ?? '—',
          ),
          paymentCompletionTime: this.formatISTDateTime(
            row?.paymentCompletionTime ?? row?.createdOn ?? row?.modifiedOn ?? row?.pickupDateTime,
          ),
          bankReference: row?.bankReference ?? '',
          paymentGroup: row?.paymentGroup ?? row?.paymentType ?? 'Online',
          bookingId: String(row?.bookingId ?? bookingID),
        };
        this.loadState = 'done';
        setTimeout(() => this.downloadReceipt(), 150);
      },
      error: () => {
        this.loadState = 'error';
      },
  ...
  downloadReceipt(): void {
    ...
    const  needGstInvoice = this.bookingDetails?.needGstInvoice;
    if (needGstInvoice) {
  ...
  private buildZeroBookingInvoiceHtml(dateStr: string): string {
    ...
    const pickupLocation =
      this.bookingDetails?.activityLocation ||
      this.bookingDetails?.pickupHub ||
      this.bookingDetails?.pickupPoint ||
      'N/A';
...
<div class="card"><strong>Company Name &amp; GSTIN</strong>${this.bookingDetails?.customerCompanyName || ''}<br> ${this.bookingDetails?.customerCompanyGST || ''}</div>
